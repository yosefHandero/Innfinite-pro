...
useEffect(() => {
  ...
  if(pageLoaded && !paymentSuccess && (!bookingRoomData || !clientSecret)) return <div className="flex items-center flex-center flex-col gap-4">
    <div className="text-rose-500">Oops! This page could not  be loaded properly...</div>
    <div className="flex items-center gap-4">
        <Button variant='outline' onClick={()=> router.push('/')}>Home</Button>
        <Button  onClick={()=> router.push('/my-bookings')}>View Bookings</Button>
    </div>
  </div>
  if(paymentSuccess) return <div className="flex items-center flex-center flex-col gap-4">
